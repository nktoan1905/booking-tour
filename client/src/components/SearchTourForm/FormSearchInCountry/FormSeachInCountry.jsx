...
const schema = yup
  .object({
    startPlace: yup.string().required("Start place is required"),
    endPlace: yup.string().required("End place is required"),
    duration: yup.string().required("Tour duration is required"),
  })
  .required();

const FormSeachInCountry = ({ cities }) => {
  ...
  const handleOnSubmit = (data, event) => {
    ...
    const newData = {
      startPlace: getValueFromLabel(data.startPlace, options),
      endPlace: getValueFromLabel(data.endPlace, options),
      duration: getValueFromLabel(data.duration, options2),
      date: dayjs(date).format("DD-MM-YYYY"),
    };
    navigate(
      `/tours/search/${newData.startPlace}/${newData.endPlace}/${newData.date}/${newData.duration}/1`
    );
  };
  useEffect(() => {
...
export default FormSeachInCountry;
