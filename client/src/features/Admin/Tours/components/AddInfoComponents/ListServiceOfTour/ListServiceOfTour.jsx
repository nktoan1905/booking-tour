...
const ListServiceOfTour = () => {
  ...
  const handleOnAddNewService = async (id) => {
    try {
      await addService(
        dispatch,
        id,
  ...
  const handleOnDeleteService = async (id, serviceId) => {
    try {
      await removeService(
        dispatch,
        id,
  ...
  return (
    <TableContainer component={Paper}>
      <Table sx={{ width: "100%" }} aria-label="simple table">
        ...
        <TableBody>
          {tourDetail.services.length > 0 ? (
            tourDetail.services.map((row, index) => (
              <TableRow
                key={row.id}
              ...
              >
                ...
                <TableCell align="center">
                  <Button
                    ...
                    onClick={() => handleOnDeleteService(tourDetail.id, row.id)}
                  >
                    X
                  </Button>
                </TableCell>
            ...
            <TableRow>
              <TableCell colSpan={6} align="center">
                Tour này chưa được add service
              </TableCell>
            </TableRow>
          )}
        </TableBody>
        ...
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            ...
            <Button
              ...
              fullWidth
              className="mt-3"
              onClick={() => handleOnAddNewService(tourDetail.id)}
            >
              Add
            </Button>
          </Typography>
...
export default ListServiceOfTour;
